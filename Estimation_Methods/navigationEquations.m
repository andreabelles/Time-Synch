...
estIMU.pos(k,1) = estIMU.pos(k-1,1) + 0.5 * (estIMU.vel(k) + estIMU.vel(k-1)) * cos(estIMU.heading(k)) * tIMU;     % North position
estIMU.pos(k,2) = estIMU.pos(k-1,2) + 0.5 * (estIMU.vel(k) + estIMU.vel(k-1)) * sin(estIMU.heading(k)) * tIMU;     % East position
