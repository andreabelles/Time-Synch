function [estIMU, pGNSS, xEKF, PEKF, xSkog, PSkog, estEKF, estSkog, estSkogPresent, PSkogPresent, measAcc, measAccInt, measAccIntLag] = ...
...
measAcc         = nan(nPts,1);
measAccCorrEKF  = nan(nPts,1);
measAccCorrSkog = nan(nPts,1);
measAccInt      = nan(nPts,1);
measAccIntLag   = nan(nPts,1);
...
for k = 2:1:nPts
    ...
    % Strapdown equations
    [estIMU(k, :)] = navigationEquations(estIMU(k-1, :), measAcc(k), Config.tIMU);
    ...
    [xSkog(:,k), PSkog(:,:,k), estSkog(k,:), measAccCorrSkog(k), estSkogPresent(k,:), PSkogPresent(:,:,k), measAccInt(k), measAccIntLag(k)] = ...
                                        skogEKF(xSkog(:,k-1),       ...
                                                PSkog(:,:,k-1),     ...
                                                estSkog(k-1,:),     ...
                                                pGNSS,              ...
